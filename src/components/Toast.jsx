  ...
  return (
    <div className="toast-stack">
      {toasts.map((t) => {
        ...
        return (
          <div key={t.id} className={`toast${t.leaving ? " toast-leaving" : ""}`} style={{
            ...
            {t.type !== "loading" && (
              <button onClick={() => onDismiss(t.id)} style={{ background: "none", border: "none", cursor: "pointer", color: TOKENS.textFaint, padding: 0, flexShrink: 0, marginTop: 1 }}>
                <X size={14} />
              </button>
            )}
          </div>
